...
CAPTCHA_API_URL = os.environ.get("CAPTCHA_API_URL", "")  # Your captcha API URL
CAPTCHA_API_KEY = os.environ.get("CAPTCHA_API_KEY", "")  # Your captcha API key
...
def auto_detect_cheapest_product(session, shop_url):
    ...
    try:
        ...
        r = session.get(url, timeout=HTTP_TIMEOUT_SHORT, verify=True)
    ...
    for handle in handles:
        try:
            ...
            r = session.get(url, timeout=HTTP_TIMEOUT_MEDIUM, verify=True)
            if r.status_code != 200:
                continue
            data = r.json()
            product = {
        ...
        if r.status_code == 200:
            ...
            for p in res:
                ...
                if not handle:
                    continue
                try:
                    pr = session.get(f"{shop_url}/products/{handle}.js", timeout=HTTP_TIMEOUT_MEDIUM, verify=True)
...
def step1_add_to_cart(session, shop_url, variant_id):
    ...
    try:
        r = session.post(add_url, json=payload, timeout=HTTP_TIMEOUT_SHORT, verify=True)
    ...
    try:
        r = session.get(checkout_url, allow_redirects=True, timeout=HTTP_TIMEOUT_SHORT, verify=True)
    except Exception as e:
...
def step2_tokenize_card(session, checkout_token, shop_url, card_data):
    ...
    for ep_url, origin, referer in endpoints:
        ...
        try:
            r = session.post(ep_url, json=payload, headers=headers, timeout=HTTP_TIMEOUT_SHORT, verify=True)
        except Exception as e:
...
def poll_for_delivery_and_expectations(session, checkout_token, session_token, merchandise_stable_id, max_attempts=7):
    ...
    for attempt in range(max_attempts):
        ...
        r = session.post(url, json=payload, headers=headers, timeout=HTTP_TIMEOUT_SHORT, verify=True)
    ...
    try:
        ...
        try:
            if '/products/' in parsed.path:
                ...
                r = session.get(product_json_url, timeout=HTTP_TIMEOUT_SHORT, verify=True)
                if r.status_code == 200:
