...
def add_to_cart(session, shop_url, variant_id):
    ...
    try:
        ...
        r = session.post(add_url, json=payload, timeout=15, verify=True)

        checkout_url = f"{shop_url}/checkout"
        r = session.get(checkout_url, allow_redirects=True, timeout=15, verify=True)
...
def tokenize_card(session, checkout_token, shop_url, card_data):
    ...
    try:
        ...
        r = session.post(ep_url, json=payload, headers=headers, timeout=15, verify=True)
...
def index():
    ...
    return jsonify({
        ...
        "example": "/index.php?key=YOUR_API_KEY&url=https://example.myshopify.com&proxy=host:port:user:pass&cc=4342580222985194|04|28|000",
        "support": "https://t.me/streetfind"
    })
